...
- **入力欄**は、キャラクターのクリックで下に開く。Enter で送り、Esc・もう一度のクリック・ほかのアプリのクリック（グローバルなマウスのモニター）で閉じる。
  ...
  - 文字の欄は AppKit の `NSTextField` にし、変換中（marked text がある間）の Enter と Esc は入力メソッドに任せる。
- **履歴**は、吹き出しか入力欄の時計のボタン、またはメニューで、キャラクターの横に開く。中身は以前の会話欄の履歴と同じで、送信欄はない。閉じるボタンか Esc で閉じる。
