...
import type { ConversationService, InterruptOutcome, SendOutcome } from './conversation.ts';
...
export class ConnectionHub {
  ...
  constructor(options: ConnectionHubOptions) {
    ...
    this.unsubscribe = options.conversation.subscribe(event => this.devices.broadcast(event.type, event.payload, event.requestId));
  }
  ...
  private receive(ws: WebSocket, connection: Connection, data: RawData, isBinary: boolean) {
    ...
    switch (type) {
      ...
      case 'conversation.interrupt': {
        ...
        return type === 'conversation.send'
          ? this.send(stream, connection.deviceId, payload, id)
          : this.interrupt(stream, payload, id);
      }
      default:
  ...
  private sync(ws: WebSocket, connection: Connection, requested: unknown, payload: Payload, requestId: string | undefined) {
    ...
    if (conversation.unavailable) {
      stream.publish('service.unavailable', { code: conversation.unavailable, deviceId }, requestId);
      return;
    }
    ...
    stream.publish('session.snapshot', { deviceId, ...conversation.snapshot() }, requestId);
  }
  ...
  private send(stream: EventStream, deviceId: string, payload: Payload, requestId: string) {
    ...
    answer(stream, this.options.conversation.send({ requestId, deviceId, text }), requestId);
  }

  private interrupt(stream: EventStream, payload: Payload, requestId: string) {
    if (typeof payload.turnId !== 'string') {
      stream.publish('command.rejected', { code: 'invalid-request' }, requestId);
      return;
    }
    answer(stream, this.options.conversation.interrupt(payload.turnId), requestId);
  }
}

function answer(stream: EventStream, outcome: SendOutcome | InterruptOutcome, requestId: string) {
  if (outcome.kind === 'unavailable') {
    stream.publish('service.unavailable', { code: outcome.code }, requestId);
  } else if (outcome.kind === 'rejected') {
    const turnId = 'turnId' in outcome ? outcome.turnId : undefined;
    stream.publish('command.rejected', { code: outcome.code, ...(turnId ? { turnId } : {}) }, requestId);
  } else {
    const { kind: _kind, ...result } = outcome;
    stream.publish('command.accepted', result, requestId);
  }
}
