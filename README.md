...
4. キャラクターをクリックすると、キャラクターの下に入力欄が出ます。Enter で送り、Shift+Enter で改行します（日本語の変換を確定する Enter では送りません）。
...
5. natsumi の最後の発言は、キャラクターの上の吹き出しに出ます。送った直後は「受付中」、返事を考えている間は「考え中」の点が出ます。
   - 長い発言は 120 文字・5 行までを出し、「続きは履歴で」から全文を見られます。
   - 届いたばかりの発言は、吹き出しの枠が強調されます。起動や再接続で戻ってきた発言は強調しません。
   - 吹き出しの × で閉じます。次の発言が来ると、また出ます。
6. 過去の会話は、入力欄の時計のボタン、キャラクターの右クリックのメニュー、メニューバーの「履歴を開く」のどれかで、キャラクターの横に開きます。
   知らせには「お知らせ」が付きます。閉じるボタンか、履歴を選んだ状態の Esc で閉じます。
7. キャラクターの右クリックのメニューには、話しかける・履歴を開く・設定…・ログアウト・終了があります。メニューバーのアイコンにも同じ項目があります。
8. キャラクターの大きさは、設定の「キャラクター」で 50% から 200% まで 25% 刻みで変えられます。すぐに反映され、次の起動でも残ります。

キャラクター・吹き出し・入力欄・履歴は、どれもほかのウインドウの上に浮かび、すべての Space と全画面表示のアプリの上にも出ます。
吹き出しや入力欄は、キャラクターと一緒に動き、画面の端では入る側に回ります。
