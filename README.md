...
4. キャラクターをクリックすると、キャラクターの下に入力欄が出ます。Enter で送ります（日本語の変換を確定する Enter では送りません）。
...
6. 過去の会話は、吹き出しか入力欄の時計のボタン、またはメニューバーの「履歴を開く」で、キャラクターの横に開きます。
   知らせには「お知らせ」が付きます。閉じるボタンか、履歴を選んだ状態の Esc で閉じます。
7. キャラクターの大きさは、設定の「キャラクター」で 50% から 200% まで 25% 刻みで変えられます。すぐに反映され、次の起動でも残ります。
