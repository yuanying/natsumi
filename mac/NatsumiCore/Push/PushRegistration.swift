...
public enum PushTidy: Equatable, Sendable {
    /// After a sync: what is still unread or unchecked stays, and nothing else.
    case synced(badge: Int, unreadReplyIds: Set<String>, unacknowledgedIds: Set<String>)
    /// After a silent push while away: what it says was read or checked goes.
    case background(BackgroundPush)

    public var badge: Int {
        switch self {
        case .synced(let badge, _, _): badge
    ...
    public func removes(_ alert: AlertPush) -> Bool {
        switch self {
        case .synced(_, let unread, let unacknowledged):
            switch alert.kind {
    ...
    static func synced(_ conversation: ConversationState) -> PushTidy {
        .synced(
            badge: conversation.unreadReplyCount + conversation.unacknowledgedNotificationIds.count,
